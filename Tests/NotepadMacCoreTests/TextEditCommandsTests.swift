...
final class TextEditCommandsTests: XCTestCase {
    ...
    func testTextEditCommandsProperCaseBlendCapitalizesWordStartsAndKeepsInteriorCase() {
        let text = "heLLo woRLD from NOTEPAD"
        let selection = NSRange(location: 0, length: (text as NSString).length)

        let result = TextEditCommands.properCaseBlendSelection(in: text, selectedRange: selection)

        XCTAssertEqual(result.text, "HeLLo WoRLD From NOTEPAD")
        XCTAssertEqual(result.selectedRange, NSRange(location: 0, length: ("HeLLo WoRLD From NOTEPAD" as NSString).length))
    }

    func testTextEditCommandsProperCaseBlendKeepsApostropheSuffixUntouched() {
        let text = "jOHN'S rock'N'roll"
        let selection = NSRange(location: 0, length: (text as NSString).length)
    ...
    func testTextEditCommandsSentenceCaseBlendCapitalizesSentenceStartsAndKeepsInteriorCase() {
        let text = "hELLO wOrLd. aNOTHER? yES! tAIL"
        let selection = NSRange(location: 0, length: (text as NSString).length)

        let result = TextEditCommands.sentenceCaseBlendSelection(in: text, selectedRange: selection)

        XCTAssertEqual(result.text, "HELLO wOrLd. ANOTHER? YES! TAIL")
        XCTAssertEqual(result.selectedRange, NSRange(location: 0, length: ("HELLO wOrLd. ANOTHER? YES! TAIL" as NSString).length))
    }
