...
public enum TextEditCommands {
    ...
    /// Sentence case (blend): uppercases the first cased character of each
    /// sentence but leaves every other character's case untouched.
    public static func sentenceCaseBlendSelection(in text: String, selectedRange: NSRange) -> TextEditCommandResult {
        convertSelectionCase(in: text, selectedRange: selectedRange, conversion: .sentenceBlend)
    }

    /// Proper Case (blend): uppercases the first cased character of each word
    /// but leaves every other character's case untouched.
    public static func properCaseBlendSelection(in text: String, selectedRange: NSRange) -> TextEditCommandResult {
        convertSelectionCase(in: text, selectedRange: selectedRange, conversion: .properBlend)
    }
    ...
    private static func convertSelectionCase(
        ...
        case .sentenceBlend:
            replacement = sentenceCasedBlend(selectedText)
        case .properBlend:
            replacement = properCasedBlend(selectedText)
        }
    ...
    private static func sentenceCasedBlend(_ text: String) -> String {
        var shouldCapitalizeNextCasedCharacter = true
        var result = ""
        ...
        for character in text {
            ...
            if hasCase, shouldCapitalizeNextCasedCharacter {
                result += uppercased
                shouldCapitalizeNextCasedCharacter = false
            } else {
                result += characterText
    ...
    private static func properCasedBlend(_ text: String) -> String {
        ...
        for (index, character) in characters.enumerated() {
            ...
            if startsWord, !followsWordApostrophe {
                result += uppercased
            } else {
                result += characterText
            }
