...
public struct AppPreferences: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        folderDropRecursiveOpen: Bool = true,
...
/// Persisted upstream-style Find/Replace dialog state (direction checkbox,
/// transparency group, Mark tab options, collapse toggle).
public struct FindDialogState: Equatable, Sendable {
    public var backwardDirection: Bool
    public var transparencyEnabled: Bool
    /// true = "On losing focus", false = "Always".
    public var transparencyOnLosingFocusOnly: Bool
    /// Window alpha applied when transparency is active (0.2...1.0).
    public var transparencyOpacity: Double
    public var markBookmarkLine: Bool
    public var markPurgeForEachSearch: Bool
    public var collapsed: Bool

    public init(
        backwardDirection: Bool = false,
        transparencyEnabled: Bool = true,
        transparencyOnLosingFocusOnly: Bool = true,
        transparencyOpacity: Double = 0.75,
        markBookmarkLine: Bool = false,
        markPurgeForEachSearch: Bool = false,
        collapsed: Bool = false
    ) {
        self.backwardDirection = backwardDirection
        self.transparencyEnabled = transparencyEnabled
        self.transparencyOnLosingFocusOnly = transparencyOnLosingFocusOnly
        self.transparencyOpacity = min(max(transparencyOpacity, 0.2), 1.0)
        self.markBookmarkLine = markBookmarkLine
        self.markPurgeForEachSearch = markPurgeForEachSearch
...
public final class PreferencesStore {
    private enum Key {
        ...
        static let findBackwardDirection = "notepadMac.find.backwardDirection"
        static let smartHighlightEnabled = "notepadMac.smartHighlightEnabled"
        static let findTransparencyEnabled = "notepadMac.find.transparencyEnabled"
        static let findTransparencyOnLosingFocus = "notepadMac.find.transparencyOnLosingFocus"
        static let findTransparencyOpacity = "notepadMac.find.transparencyOpacity"
        static let findMarkBookmarkLine = "notepadMac.find.markBookmarkLine"
        static let findMarkPurgeForEachSearch = "notepadMac.find.markPurgeForEachSearch"
        static let findDialogCollapsed = "notepadMac.find.dialogCollapsed"
    ...
    public func load() -> AppPreferences {
        AppPreferences(
            ...
            folderDropRecursiveOpen: defaults.object(forKey: Key.folderDropRecursiveOpen) as? Bool ?? true,
    ...
    /// Smart highlighting (highlight all occurrences of the selected
    /// word) is enabled by default, matching upstream Notepad++.
    public func loadSmartHighlightEnabled() -> Bool {
        defaults.object(forKey: Key.smartHighlightEnabled) as? Bool ?? true
    }

    public func saveSmartHighlightEnabled(_ enabled: Bool) {
        defaults.set(enabled, forKey: Key.smartHighlightEnabled)
        defaults.synchronize()
    }

    public func loadFindDialogState() -> FindDialogState {
        FindDialogState(
            backwardDirection: defaults.bool(forKey: Key.findBackwardDirection),
            transparencyEnabled: defaults.object(forKey: Key.findTransparencyEnabled) as? Bool ?? true,
            transparencyOnLosingFocusOnly: defaults.object(forKey: Key.findTransparencyOnLosingFocus) as? Bool ?? true,
            transparencyOpacity: defaults.object(forKey: Key.findTransparencyOpacity) as? Double ?? 0.75,
            markBookmarkLine: defaults.bool(forKey: Key.findMarkBookmarkLine),
            markPurgeForEachSearch: defaults.bool(forKey: Key.findMarkPurgeForEachSearch),
            collapsed: defaults.bool(forKey: Key.findDialogCollapsed)
        )
    }

    public func saveFindDialogState(_ state: FindDialogState) {
        defaults.set(state.backwardDirection, forKey: Key.findBackwardDirection)
        defaults.set(state.transparencyEnabled, forKey: Key.findTransparencyEnabled)
        defaults.set(state.transparencyOnLosingFocusOnly, forKey: Key.findTransparencyOnLosingFocus)
        defaults.set(state.transparencyOpacity, forKey: Key.findTransparencyOpacity)
        defaults.set(state.markBookmarkLine, forKey: Key.findMarkBookmarkLine)
        defaults.set(state.markPurgeForEachSearch, forKey: Key.findMarkPurgeForEachSearch)
        defaults.set(state.collapsed, forKey: Key.findDialogCollapsed)
        defaults.synchronize()
    }
