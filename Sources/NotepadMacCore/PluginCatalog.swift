...
public struct PluginInstallationResult: Equatable, Sendable {
    ...
    /// Version of the previously installed copy when `action == .updated`.
...
public enum PluginInstallationError: Error, Equatable, Sendable {
    ...
    case archiveExtractionFailed(URL, reason: String)
    case archiveMissingManifest(URL)
}
...
public struct PluginCatalog: Equatable, Sendable {
    ...
    public static func installNativePlugin(
        ...
        let action: PluginInstallationAction = fileManager.fileExists(atPath: destinationURL.path)
            ? .updated
            : .installed
        let previousVersion: String? = action == .updated
    ...
    /// Installs or updates a native plugin from a .zip archive (for
    /// "Import plugin(s)..." with archives and URL installs). The archive
    /// is extracted with /usr/bin/ditto, the manifest folder is located
    /// (tolerating a single wrapping folder and __MACOSX entries), and the
    /// regular directory install path is reused.
    public static func installNativePlugin(
        ...
        let extractRoot = fileManager.temporaryDirectory
            .appending(path: "notepad-mac-plugin-archive-\(UUID().uuidString)")
            .standardizedFileURL
        ...
        process.arguments = ["-x", "-k", archiveURL.path, extractRoot.path]
        let errorPipe = Pipe()
        ...
        do {
            ...
            throw PluginInstallationError.archiveExtractionFailed(
                archiveURL, reason: error.localizedDescription)
        }
        ...
        guard process.terminationStatus == 0 else {
            ...
            throw PluginInstallationError.archiveExtractionFailed(
                archiveURL,
                reason: reason.isEmpty ? "ditto exited with status \(process.terminationStatus)" : reason
            )
        }

        guard let pluginRoot = locateManifestDirectory(in: extractRoot, fileManager: fileManager) else {
            throw PluginInstallationError.archiveMissingManifest(archiveURL)
        }
        return try installNativePlugin(
            from: pluginRoot,
            into: userPluginDirectoryURL,
            fileManager: fileManager
        )
    }

    /// Finds the shallowest directory containing notepad-mac-plugin.json
    /// under `root`, ignoring hidden entries and __MACOSX metadata.
    static func locateManifestDirectory(in root: URL, fileManager: FileManager = .default) -> URL? {
        var queue: [URL] = [root.standardizedFileURL]
        var depth = 0
        while !queue.isEmpty, depth <= 4 {
            var nextLevel: [URL] = []
            var candidates: [URL] = []
            for directory in queue {
                if fileManager.fileExists(atPath: directory.appending(path: "notepad-mac-plugin.json").path) {
                ...
                for child in children {
                    ...
                    guard name != "__MACOSX" else { continue }
