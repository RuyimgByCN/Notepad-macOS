...
public enum TextSearch {
    ...
    public static func replaceNext(
        ...
        // Regex mode substitutes capture-group references in the replacement
        // through the Boost engine (format_all), matching upstream Notepad++
        // behavior for single replacements.
        if options.searchMode == .regex,
           let engine = makeBoostEngine(query, options: options),
           engine.firstMatch(
               in: text, range: range,
               dotMatchesLineSeparators: options.dotMatchesLineSeparators
           ) != nil,
           let substituted = engine.format(replacement: replacement) {
            let replacedText = (text as NSString).replacingCharacters(in: range, with: substituted)
            return Replacement(text: replacedText, replacedRange: NSRange(location: range.location, length: substituted.utf16.count))
        }
    ...
    public static func replaceAll(
        ...
        // "In selection": replace only inside the constrained range, then
        // splice the result back into the full text (upstream behavior).
        if let constraint = options.searchRange {
            let nsText = text as NSString
            let lo = clamped(constraint.location, to: nsText.length)
            let hi = clamped(NSMaxRange(constraint), to: nsText.length)
            guard lo < hi else { return ReplaceAllResult(text: text, count: 0) }
            let scopedRange = NSRange(location: lo, length: hi - lo)
            var scopedOptions = options
            scopedOptions.searchRange = nil
            let scoped = replaceAll(
                query,
                with: replacement,
                in: nsText.substring(with: scopedRange),
                options: scopedOptions
            )
            let merged = nsText.replacingCharacters(in: scopedRange, with: scoped.text)
            return ReplaceAllResult(text: merged, count: scoped.count)
        }

        // Regex replace through the Boost engine: each match is formatted
        // immediately (format_all) so backreferences, conditionals, and case
        // operators behave exactly like upstream.
        if options.searchMode == .regex {
            guard let engine = makeBoostEngine(query, options: options) else {
                return ReplaceAllResult(text: text, count: 0)
            }
            let nsText = text as NSString
            var pieces: [String] = []
            var copied = 0
            var count = 0
            var window = NSRange(location: 0, length: nsText.length)
            while window.location <= nsText.length {
                guard let match = engine.firstMatch(
                    in: text, range: window,
                    dotMatchesLineSeparators: options.dotMatchesLineSeparators
                ) else { break }
                let advance = max(NSMaxRange(match.range), match.range.location + 1)
                if !options.wholeWord || isWholeWord(match.range, in: nsText) {
                    let formatted = engine.format(replacement: replacement) ?? replacement
                    pieces.append(nsText.substring(
                        with: NSRange(location: copied, length: match.range.location - copied)))
                    pieces.append(formatted)
                    copied = NSMaxRange(match.range)
                    count += 1
                }
                guard advance <= nsText.length else { break }
                window = NSRange(location: advance, length: nsText.length - advance)
            }
            pieces.append(nsText.substring(from: copied))
            return ReplaceAllResult(text: pieces.joined(), count: count)
        }
    ...
    private static func findRegex(_ pattern: String, in text: NSString, searchRange: NSRange, options: Options, direction: Direction) -> NSRange? {
        guard let engine = makeBoostEngine(pattern, options: options) else { return nil }

        let matches = engine.allMatches(
            in: text as String,
            range: searchRange,
            dotMatchesLineSeparators: options.dotMatchesLineSeparators
        )
    ...
    /// Builds the upstream-compatible Boost regex engine for a pattern.
    /// Returns nil when the pattern fails to compile.
    private static func makeBoostEngine(_ pattern: String, options: Options) -> BoostRegexEngine? {
        try? BoostRegexEngine(pattern: pattern, matchCase: options.matchCase)
    }

    /// Returns a human-readable description of why a regex pattern cannot be
    /// used, or nil when the pattern is valid. Intended for find-panel UI.
    public static func regexPatternProblem(_ pattern: String) -> String? {
        BoostRegexEngine.patternProblem(pattern)
    }
