...
public enum ShortcutsXMLCodec {
    ...
    public struct ValidationReport: Equatable, Sendable {
        ...
        public init(shortcutCount: Int, issues: [ValidationIssue]) {
            self.shortcutCount = shortcutCount
            self.issues = issues
        }
    }

    /// Validates a shortcuts.xml payload: XML well-formedness, the
    /// NotepadPlus/InternalCommands structure, per-Shortcut attribute values,
    /// and duplicate key-combination conflicts.
    public static func validate(_ data: Data) -> ValidationReport {
        let validator = ShortcutsXMLValidator(data: data)
...
private final class ShortcutsXMLValidator: NSObject, XMLParserDelegate {
    private let data: Data
    private var issues: [ShortcutsXMLCodec.ValidationIssue] = []
    private var sawRoot = false
    private var sawInternalCommands = false
    private var insideInternal = false
    private var shortcutCount = 0
    private var seenCombinations: [String: String] = [:]

    init(data: Data) { self.data = data }

    func validate() -> ShortcutsXMLCodec.ValidationReport {
        let parser = XMLParser(data: data)
        parser.delegate = self
        if !parser.parse() {
            let line = parser.lineNumber
            let reason = parser.parserError?.localizedDescription ?? "malformed XML"
            issues.append(.init(message: "Line \(line): XML parse error (\(reason))."))
        }
        if !sawRoot {
            issues.append(.init(message: "Missing <NotepadPlus> root element."))
        } else if !sawInternalCommands {
            issues.append(.init(message: "Missing <InternalCommands> section."))
        }
        return ShortcutsXMLCodec.ValidationReport(shortcutCount: shortcutCount, issues: issues)
    }
    ...
    func parser(_ parser: XMLParser, didStartElement elementName: String,
        ...
        case "NotepadPlus":
            sawRoot = true
        case "InternalCommands":
            sawInternalCommands = true
            insideInternal = true
        case "Shortcut":
            guard insideInternal else {
                issues.append(.init(message: "Line \(parser.lineNumber): <Shortcut> outside <InternalCommands> is ignored."))
                return
            }
            shortcutCount += 1
            validateShortcut(attrs, line: parser.lineNumber)
        default:
            break
        }
    }
    ...
    private func validateShortcut(_ attrs: [String: String], line: Int) {
        let title = attrs["menuItemTitle"] ?? ""
        let label = title.isEmpty ? "(no menuItemTitle)" : "\"\(title)\""

        for flag in ["Ctrl", "Alt", "Shift"] {
            if let value = attrs[flag], value != "yes", value != "no" {
                issues.append(.init(message: "Line \(line): \(label) has invalid \(flag)=\"\(value)\" (expected yes/no)."))
            }
        }

        guard let keyText = attrs["Key"] else {
            issues.append(.init(message: "Line \(line): \(label) is missing the Key attribute."))
            return
        }
        guard let keyCode = Int(keyText), keyCode >= 0 else {
            issues.append(.init(message: "Line \(line): \(label) has non-numeric Key=\"\(keyText)\"."))
            return
        }
        if keyCode == 0 {
            issues.append(.init(message: "Line \(line): \(label) has Key=\"0\" (unassigned)."))
            return
        }
        if ShortcutsXMLCodec.vkToNSKey(keyCode) == nil {
            issues.append(.init(message: "Line \(line): \(label) has unmappable Key=\"\(keyCode)\" on macOS."))
        }

        let combination = "\(attrs["Ctrl"] ?? "no")|\(attrs["Alt"] ?? "no")|\(attrs["Shift"] ?? "no")|\(keyCode)"
        if let previousTitle = seenCombinations[combination], !title.isEmpty, previousTitle != title {
            issues.append(.init(message: "Line \(line): \(label) conflicts with \"\(previousTitle)\" (same key combination)."))
        } else {
            seenCombinations[combination] = title
        }
    }
}
