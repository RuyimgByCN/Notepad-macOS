...
final class ShortcutMapperPanelController: NSWindowController, NSTableViewDataSource, NSTableViewDelegate {
    ...
    private func configureContent() {
        ...
        let exportImportEntries: [(String, Selector)] = [
            (Localization.string(.shortcutMapperExportJSON, default: "Export Shortcuts (JSON)..."), #selector(exportShortcuts(_:))),
            (Localization.string(.shortcutMapperImportJSON, default: "Import Shortcuts (JSON)..."), #selector(importShortcuts(_:))),
            (Localization.string(.shortcutMapperExportXML, default: "Export Shortcuts (XML)..."), #selector(exportShortcutsXML(_:))),
            (Localization.string(.shortcutMapperImportXML, default: "Import Shortcuts (XML)..."), #selector(importShortcutsXML(_:))),
        ]
        for (entryTitle, entryAction) in exportImportEntries {
            exportImportButton.addItem(withTitle: entryTitle)
            exportImportButton.lastItem?.target = self
            exportImportButton.lastItem?.action = entryAction
        }
