...
final class EditorWindowController: NSWindowController, NSWindowDelegate, NSMenuItemValidation, NSToolbarItemValidation, NSDraggingDestination {
    ...
    /// Fired when the dirty flag flips, so the host can refresh tab-bar
    /// saved/unsaved icons immediately instead of on the next activation.
    var onDirtyStateChange: (() -> Void)?
    var onSessionStateChange: (() -> Void)?
    var onActivate: (() -> Void)?
    var onTabSelect: ((EditorTabIdentity) -> Void)?
    /// Document Peeker: forwarded tab hover events (nil identity = hide).
    var onTabPeek: ((EditorTabIdentity?, NSRect) -> Void)?
    ...
    /// Hosts the main editor surface and, when "Clone to Other View" is
    /// active, a second surface sharing the same Scintilla document.
    private let editorSplitView = NSSplitView()
    private(set) var clonedSurface: EditorSurface?

    /// Second pane hosting ANOTHER document's shared buffer
    /// (upstream View > Move to Other View). The hosted document keeps its
    /// own (hidden) controller, which continues to track dirty state and
    /// perform saves through the shared Scintilla document.
    private(set) var hostedSurface: EditorSurface?
    private(set) weak var hostedController: EditorWindowController?
    /// Set on a parked controller while its buffer is shown in another
    /// window's second pane.
    weak var hostingController: EditorWindowController?
    ...
    private lazy var enablesSmartHighlight = preferencesStore.loadSmartHighlightEnabled()
    ...
    private var beginColumnSelectPosition: Int?
    ...
    func windowWillClose(_ notification: Notification) {
        removeClonedView()
        // Closing a host window returns the hosted document to its own window.
        releaseHostedDocument(reactivateReleased: false)
        // Closing a parked document collapses the host's second pane.
        hostingController?.handleParkedControllerClosed(self)
    ...
    @objc private func editorTextDidChange(_ notification: Notification) {
        ...
        // edits) manage the dirty flag explicitly at their call sites; only
        // direct user editing marks the document dirty here. The Scintilla
        // surface posts this notification deferred, so trusting it for
        // programmatic changes would override load()'s isDirty = false.
        let isProgrammaticChange =
            notification.userInfo?[EditorSurfaceNotificationKey.programmaticTextChange] as? Bool == true
    ...
    @objc func saveDocument(_ sender: Any?) {
        // When the hosted second pane has focus, the save belongs to the
        // hosted document's own controller (upstream per-view semantics).
        if hostedPaneHasFocus, let hosted = hostedController {
    ...
    @objc func encodeInEncoding(_ sender: Any?) {
        ...
        do {
            ...
            guard let text = TextFileCodec.decode(data, encoding: option.encoding) else {
                let alert = NSAlert()
    ...
    @objc func toggleSmartHighlight(_ sender: Any?) {
        ...
        preferencesStore.saveSmartHighlightEnabled(enablesSmartHighlight)
    ...
    private func applyUrlHighlights() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            DispatchQueue.main.async { [weak self] in
                self?.editorSurface.applyUrlHighlights(ranges: ranges, style: style)
            }
        }
    }
    ...
    private func updateSmartHighlight() {
        guard enablesSmartHighlight else {
            editorSurface.clearSmartHighlight()
            return
        }
        ...
        let isSingleLine = !token.contains("\n") && !token.contains("\r")
        let isReasonableLength = token.utf16.count <= 512
        let hasContent = !token.trimmingCharacters(in: .whitespaces).isEmpty
        guard isSingleLine, isReasonableLength, hasContent else {
            editorSurface.clearSmartHighlight()
            return
        }
    ...
    /// Edit > Begin/End Select in Column Mode: the first invocation records the
    /// caret as the column anchor; the second applies a rectangular selection
    /// from the anchor to the current caret. The NSTextView fallback degrades
    /// to a contiguous selection because it has no column selection support.
    @objc func beginOrEndSelectInColumnMode(_ sender: Any?) {
        if let beginPos = beginColumnSelectPosition {
            let textLength = (editorSurface.text as NSString).length
            let anchor = min(beginPos, textLength)
            let caret = min(editorSurface.selectedRange.location, textLength)
            let applied = editorSurface.applyRectangularSelection(
                RectangularSelectionLiveMetadata(
            ...
            if !applied {
                ...
                editorSurface.setSelectedRange(NSRange(location: start, length: abs(caret - anchor)))
            }
            beginColumnSelectPosition = nil
        } else {
            beginColumnSelectPosition = editorSurface.selectedRange.location
        }
        updateStatus()
    }
    ...
    // MARK: - Cloned view (View > Clone to Other View)

    /// View > Clone to Other View: toggles a second editor pane that shares
    /// this document through Scintilla's document pointer. Both panes edit the
    /// same buffer with independent carets, selections, and scroll positions.
    @objc func toggleCloneToOtherView(_ sender: Any?) {
        if clonedSurface != nil {
            removeClonedView()
            return
        }
        guard hostedSurface == nil else {
            beepIfEnabled()
            return
        }
        guard let documentPointer = editorSurface.documentPointer else {
            beepIfEnabled()
            return
        }
        let clone = EditorSurfaceFactory.make()
        guard clone.setDocumentPointer(documentPointer) else {
            beepIfEnabled()
            return
        }
        clonedSurface = clone
        editorSplitView.addArrangedSubview(clone.view)
        configureClonedSurface(clone)
        editorSplitView.layoutSubtreeIfNeeded()
        editorSplitView.setPosition(editorSplitView.bounds.width / 2, ofDividerAt: 0)
    }

    /// View > Focus on Another View: moves keyboard focus to the other pane.
    @objc func focusOtherView(_ sender: Any?) {
        guard let pane = clonedSurface ?? hostedSurface else {
            beepIfEnabled()
            return
        }
        let paneHasFocus = (window?.firstResponder as? NSView)?.isDescendant(of: pane.view) == true
        window?.makeFirstResponder(paneHasFocus ? editorSurface.firstResponder : pane.firstResponder)
    }

    // MARK: - Hosted second view (View > Move to Other View)

    var secondPaneExists: Bool { clonedSurface != nil || hostedSurface != nil }

    var hostedPaneHasFocus: Bool {
        guard let pane = hostedSurface else { return false }
        return (window?.firstResponder as? NSView)?.isDescendant(of: pane.view) == true
    }

    /// Shows `other`'s buffer in this window's second pane and parks
    /// `other`'s window. Both panes stay live: the parked controller keeps
    /// receiving document notifications through the shared buffer.
    func hostDocumentInOtherView(from other: EditorWindowController) -> Bool {
        guard other !== self,
              clonedSurface == nil,
              hostedSurface == nil,
              other.hostingController == nil,
              other.hostedController == nil,
              other.clonedSurface == nil,
              let documentPointer = other.editorSurface.documentPointer
        else { return false }

        let pane = EditorSurfaceFactory.make()
        guard pane.setDocumentPointer(documentPointer) else { return false }

        hostedSurface = pane
        hostedController = other
        other.hostingController = self
        editorSplitView.addArrangedSubview(pane.view)
        other.configureClonedSurface(pane)
        editorSplitView.layoutSubtreeIfNeeded()
        editorSplitView.setPosition(editorSplitView.bounds.width / 2, ofDividerAt: 0)
        other.window?.orderOut(nil)
        window?.makeFirstResponder(pane.firstResponder)
        return true
    }

    /// Collapses the hosted pane and returns the released controller so
    /// the caller can decide whether to bring its window back on screen.
    @discardableResult
    func releaseHostedDocument(reactivateReleased: Bool) -> EditorWindowController? {
        guard let pane = hostedSurface else { return nil }
        if hostedPaneHasFocus {
            window?.makeFirstResponder(editorSurface.firstResponder)
        }
        pane.detachFromSharedDocument()
        pane.view.removeFromSuperview()
        hostedSurface = nil
        let released = hostedController
        hostedController = nil
        released?.hostingController = nil
        _ = reactivateReleased
        return released
    }

    /// Called on the host when a parked document's controller closes.
    func handleParkedControllerClosed(_ parked: EditorWindowController) {
        guard parked === hostedController, let pane = hostedSurface else { return }
        if hostedPaneHasFocus {
            window?.makeFirstResponder(editorSurface.firstResponder)
        }
        pane.detachFromSharedDocument()
        ...
        hostedSurface = nil
        hostedController = nil
        parked.hostingController = nil
    }

    func focusHostedPane() {
        guard let pane = hostedSurface else { return }
        window?.makeFirstResponder(pane.firstResponder)
    }

    private func removeClonedView() {
        guard let clone = clonedSurface else { return }
        if (window?.firstResponder as? NSView)?.isDescendant(of: clone.view) == true {
            window?.makeFirstResponder(editorSurface.firstResponder)
        }
        clone.detachFromSharedDocument()
        clone.view.removeFromSuperview()
        clonedSurface = nil
    }

    /// Mirrors the main surface's display configuration onto the cloned pane.
    /// Document-level state (text, lexer, read-only) is shared automatically;
    /// view-level state (font, wrap, margins, styles) must be applied per view.
    private func configureClonedSurface(_ surface: EditorSurface) {
        surface.applyFont(name: editorFontName, size: fontSize, bold: editorFontBold)
        let width = window?.contentView?.bounds.width ?? 0
        surface.applyLineWrapping(wrapsLines, width: width)
        if wrapsLines, surface.supportsAdvancedViewOptions {
            surface.applyWordWrapMode(wordWrapMode)
        }
        surface.applyLineNumberMargin(showsLineNumberMargin)
        surface.applyBookmarkMarginVisible(showsBookmarkMargin)
        surface.applyCodeFolding(enableCodeFolding && foldMarginStyle != FoldMarginStyle.none.rawValue)
        surface.applyFoldMarginStyle(foldMarginStyle)
        surface.applyCurrentLineHighlight(highlightsCurrentLine)
        surface.applyShowWhitespace(whitespaceMode != .invisible)
        surface.applyShowEOL(showsEOL)
        if !isLargeFile || !preferencesStore.load().largeFileSuppressSyntaxHighlight {
            surface.applyHighlight(
                language: language,
                styleCatalog: styleCatalog,
    ...
    /// Edit > Copy Binary Content: copies the selection's raw bytes (in the
    /// document encoding) to a custom pasteboard type, preserving NUL bytes.
    @objc func copyBinaryContent(_ sender: Any?) {
        guard let data = selectedBinaryData() else {
            beepIfEnabled()
            return
        }
        writeBinaryToPasteboard(data)
    }

    /// Edit > Cut Binary Content: Copy Binary Content plus selection removal.
    @objc func cutBinaryContent(_ sender: Any?) {
        guard let data = selectedBinaryData() else {
            beepIfEnabled()
            return
        }
    ...
    /// Edit > Paste Binary Content: inserts the binary pasteboard payload,
    /// decoding it with the document encoding (ISO Latin-1 fallback keeps
    /// arbitrary bytes byte-for-byte reversible).
    @objc func pasteBinaryContent(_ sender: Any?) {
        let pb = NSPasteboard.general
        guard let data = pb.data(forType: NSPasteboard.PasteboardType(BinaryClipboard.pasteboardType)),
              let text = BinaryClipboard.decode(data: data, encoding: encoding)
        else {
            beepIfEnabled()
            return
        }
    ...
    private func selectedBinaryData() -> Data? {
        let selection = editorSurface.selectedRange
        guard selection.length > 0 else { return nil }
        let nsText = editorSurface.text as NSString
        guard NSMaxRange(selection) <= nsText.length else { return nil }
        let selectedText = nsText.substring(with: selection)
        return BinaryClipboard.encode(selectedText: selectedText, encoding: encoding)
    }

    private func writeBinaryToPasteboard(_ data: Data) {
        let pb = NSPasteboard.general
        pb.clearContents()
        pb.setData(data, forType: NSPasteboard.PasteboardType(BinaryClipboard.pasteboardType))
        // Also expose a plain-text rendering for interoperability with other apps.
        if let text = BinaryClipboard.decode(data: data, encoding: encoding) {
    ...
    @objc func showFindInFilesPanel(_ sender: Any?) {
        // Show the Find dialog's "Find in Files" tab (upstream behavior:
        // same window, not a separate panel).
        findPanel.show(tab: .findInFiles)
    }
    ...
    @objc func properCaseBlendSelection(_ sender: Any?) {
        applyEditCommandResult(
            TextEditCommands.properCaseBlendSelection(in: editorSurface.text, selectedRange: editorSurface.selectedRange)
        )
    }

    @objc func sentenceCaseBlendSelection(_ sender: Any?) {
        applyEditCommandResult(
            TextEditCommands.sentenceCaseBlendSelection(in: editorSurface.text, selectedRange: editorSurface.selectedRange)
        )
    }
    ...
    /// Edit > Auto-Completion > Function Parameters Next Hint: cycles the
    /// visible call tip to the next overloaded signature.
    @objc func showNextCallTipSignature(_ sender: Any?) {
        guard callTipPanel.canCycleSignatures else {
            beepIfEnabled()
            return
        }
        callTipPanel.showNextSignature()
    }

    /// Edit > Auto-Completion > Function Parameters Previous Hint: cycles the
    /// visible call tip to the previous overloaded signature.
    @objc func showPreviousCallTipSignature(_ sender: Any?) {
        guard callTipPanel.canCycleSignatures else {
            beepIfEnabled()
            return
        }
        callTipPanel.showPreviousSignature()
    }
    ...
    func validateMenuItem(_ menuItem: NSMenuItem) -> Bool {
        ...
        case #selector(beginOrEndSelect(_:)):
            ...
            return true
        case #selector(toggleCloneToOtherView(_:)):
            menuItem.state = clonedSurface == nil ? .off : .on
            return editorSurface.documentPointer != nil
        case #selector(focusOtherView(_:)):
            return clonedSurface != nil
        case #selector(beginOrEndSelectInColumnMode(_:)):
            menuItem.title = beginColumnSelectPosition == nil
                ? Localization.string(.editBeginSelectColumnMode, default: "Begin Select in Column Mode")
                : Localization.string(.editEndSelectColumnMode, default: "End Select in Column Mode")
            return true
        ...
        case #selector(showNextCallTipSignature(_:)),
            #selector(showPreviousCallTipSignature(_:)):
            return callTipPanel.canCycleSignatures
        case #selector(copyBinaryContent(_:)):
            return editorSurface.selectedRange.length > 0
        case #selector(cutBinaryContent(_:)):
            return editorSurface.selectedRange.length > 0 && !editorSurface.isReadOnly
        case #selector(pasteBinaryContent(_:)):
            let binaryType = NSPasteboard.PasteboardType(BinaryClipboard.pasteboardType)
            return NSPasteboard.general.types?.contains(binaryType) == true && !editorSurface.isReadOnly
        ...
        case #selector(uppercaseSelection(_:)),
            ...
            #selector(properCaseBlendSelection(_:)),
            #selector(sentenceCaseSelection(_:)),
            #selector(sentenceCaseBlendSelection(_:)),
    ...
    private func configureContent() {
        ...
        tabBarView.onTabPeek = { [weak self] identity, rect in self?.onTabPeek?(identity, rect) }
        ...
        editorSplitView.addArrangedSubview(editorSurface.view)

        rootView.addSubview(toolbarRow)
        rootView.addSubview(tabBarView)
        rootView.addSubview(editorSplitView)
        ...
        NSLayoutConstraint.activate([
            ...
            editorSplitView.leadingAnchor.constraint(equalTo: rootView.leadingAnchor),
            editorSplitView.trailingAnchor.constraint(equalTo: rootView.trailingAnchor),
            editorSplitView.topAnchor.constraint(equalTo: tabBarView.bottomAnchor),
            editorSplitView.bottomAnchor.constraint(equalTo: statusField.topAnchor),
    ...
    /// Applies a validated plugin edit script to this editor's buffer.
    /// Returns an error description, or nil on success.
    func applyPluginEditScript(_ script: PluginEditScript) -> String? {
        guard !editorSurface.isReadOnly else {
            return Localization.string(.pluginsEditScriptReadOnly, default: "Document is read-only.")
        }
        do {
            let result = try script.apply(to: editorSurface.text, selection: editorSurface.selectedRange)
