...
final class EditorTabBarView: NSView {
    ...
    /// Document Peeker: called with (identity, tab frame in screen
    /// coordinates) after the pointer dwells on a tab, and with nil when
    ...
    func handleTabPeekHover(_ button: NSView?, identity: EditorTabIdentity, hovering: Bool) {
        peekGeneration += 1
        guard onTabPeek != nil else { return }
        if hovering, let button {
            let generation = peekGeneration
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.45) { [weak self, weak button] in
                guard let self, self.peekGeneration == generation,
                      let button, let window = button.window else { return }
                let rectInWindow = button.convert(button.bounds, to: nil)
                let screenRect = window.convertToScreen(rectInWindow)
                self.onTabPeek?(identity, screenRect)
            }
        } else {
            onTabPeek?(nil, .zero)
        }
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        if let btn = tabButtons.first(where: { $0.frame.contains(docLoc) }) {
            beginTabDrag(btn, with: event)
        }
        super.mouseDown(with: event)
    }

    override func mouseDragged(with event: NSEvent) {
        continueTabDrag(with: event)
    }

    override func mouseUp(with event: NSEvent) {
        if mouseDownInEmptyArea { mouseDownInEmptyArea = false; return }
        if !finishTabDrag(with: event) {
            super.mouseUp(with: event)
        }
    }

    private func beginTabDrag(_ btn: EditorTabButton, with event: NSEvent) {
        guard !lockDragDrop else { return }
        let docLoc = documentView.convert(event.locationInWindow, from: nil)
        draggedButton = btn
        dragStartX = docLoc.x
    ...
    private func continueTabDrag(with event: NSEvent) {
        guard let btn = draggedButton, !lockDragDrop else { return }
        let docLoc = documentView.convert(event.locationInWindow, from: nil)
        let delta = docLoc.x - dragStartX
        guard isDraggingTab || abs(delta) >= dragActivationDistance else { return }
        isDraggingTab = true
        btn.alphaValue = 0.6
    ...
    @discardableResult
    private func finishTabDrag(with event: NSEvent) -> Bool {
    ...
    private func rebuildTabs() {
        ...
        for (index, item) in state.items.enumerated() {
            ...
            btn.onHoverPeek = { [weak self, weak btn] hovering in
                self?.handleTabPeekHover(btn, identity: item.identity, hovering: hovering)
            }
            ...
            btn.onDragStart = { [weak self] button, event in self?.beginTabDrag(button, with: event) }
            btn.onDragMove = { [weak self] _, event in self?.continueTabDrag(with: event) }
            btn.onDragEnd = { [weak self] _, event in self?.finishTabDrag(with: event) ?? false }
...
final class EditorTabButton: NSView {
    ...
    /// Document Peeker hover hook: true on dwell start, false on exit/click.
    var onHoverPeek: ((Bool) -> Void)?
    private let onSelect: () -> Void
    private let onClose: () -> Void
    private let onContextAction: (TabContextAction) -> Void
    var onRename: (() -> Void)?
    var onDragStart: ((EditorTabButton, NSEvent) -> Void)?
    var onDragMove: ((EditorTabButton, NSEvent) -> Void)?
    var onDragEnd: ((EditorTabButton, NSEvent) -> Bool)?
    ...
    override func mouseDragged(with event: NSEvent) {
        guard isTrackingPrimaryClick else {
    ...
    override func mouseUp(with event: NSEvent) {
        ...
        let completedDrag = onDragEnd?(self, event) ?? false
        if !completedDrag {
            onSelect()
        }
        onHoverPeek?(false)
