...
private var appearanceObservation: NSKeyValueObservation?
    ...
    private let documentPeeker = DocumentPeekerController()
    ...
    private lazy var pluginsPanel = PluginsPanelController(
        ...
        editScriptApplier: { [weak self] script in
            guard let controller = self?.activeEditorController() else {
                return Localization.string(.pluginsEditScriptNoEditor, default: "No active editor to apply plugin edits to.")
            }
            return controller.applyPluginEditScript(script)
        }
    )
    ...
    func validateMenuItem(_ menuItem: NSMenuItem) -> Bool {
        ...
        case #selector(openInNewInstance(_:)), #selector(moveToNewInstance(_:)):
        ...
        case #selector(showDocumentList(_:)):
            return !windows.isEmpty
        case #selector(moveToOtherView(_:)):
            if let current = activeEditorController() {
                return current.hostedController != nil
                    || (windows.count >= 2 && !current.secondPaneExists)
            }
            return false
        default:
            return true
    ...
    /// File > New and Paste: create an untitled document and insert the current
    /// clipboard text. If the clipboard has no plain text, only the new document
    /// is created (matching upstream Notepad++ behavior).
    @objc func newDocumentAndPaste(_ sender: Any?) {
        let controller = makeNewDocument()
        if NSPasteboard.general.string(forType: .string) != nil {
            controller.pasteAsPlainText(nil)
        }
    }
    ...
    @objc func openDocument(_ sender: Any?) {
        ...
        let readOnlyBox = NSButton(checkboxWithTitle: Localization.string(.fileOpenAsReadOnly, default: "Open as Read Only"), target: nil, action: nil)
    ...
    func openURLs(_ urls: [URL], batchThreshold: Int = 200, readOnly: Bool = false) {
        ...
        if urls.count > batchThreshold {
            let alert = NSAlert()
            alert.messageText = String(
                format: Localization.string(.fileOpenManyTitle, default: "Open %d Files?"),
                urls.count
            )
            alert.informativeText = String(
                format: Localization.string(
                    .fileOpenManyMessage,
                    default: "You are about to open %d files. This may take a while and use significant memory."
                ),
                urls.count
            )
            alert.addButton(withTitle: Localization.string(.fileOpenManyOpenAll, default: "Open All"))
            alert.addButton(withTitle: Localization.string(.alertCancel, default: "Cancel"))
    ...
    @objc func saveAllDocuments(_ sender: Any?) {
        ...
        if prefs.saveAllConfirm && dirtyCount > 0 {
            let alert = NSAlert()
            alert.messageText = String(
                format: Localization.string(.fileSaveAllConfirmTitle, default: "Save %d modified file(s)?"),
                dirtyCount
            )
            alert.informativeText = Localization.string(
                .fileSaveAllConfirmMessage,
                default: "All unsaved changes will be written to disk."
            )
            alert.addButton(withTitle: Localization.string(.fileSaveAllConfirmButton, default: "Save All"))
            alert.addButton(withTitle: Localization.string(.alertCancel, default: "Cancel"))
            guard alert.runModal() == .alertFirstButtonReturn else { return }
        }
    ...
    /// Window > Open in New Instance: launches a separate app instance with
    /// the current file. The new instance runs with -nosession so the two
    /// instances do not fight over session state.
    @objc func openInNewInstance(_ sender: Any?) {
        launchNewInstanceWithActiveDocument(closeCurrentTab: false)
    }

    /// Window > Move to New Instance: Open in New Instance plus closing the
    /// document in this instance.
    @objc func moveToNewInstance(_ sender: Any?) {
        launchNewInstanceWithActiveDocument(closeCurrentTab: true)
    }

    private func launchNewInstanceWithActiveDocument(closeCurrentTab: Bool) {
        guard let controller = activeEditorController(),
        ...
        if controller.hasUnsavedChanges {
            ...
            alert.messageText = fileURL.lastPathComponent
            alert.informativeText = Localization.string(
                .windowNewInstanceUnsaved,
                default: "Save the document before opening it in a new instance."
            )
            alert.runModal()
            return
        }
        ...
        configuration.arguments = ["-nosession", fileURL.path]
        let identity = controller.tabIdentity
            ...
            DispatchQueue.main.async {
                ...
                if let error {
                    ...
                    alert.messageText = Localization.string(
                        .windowNewInstanceLaunchFailed,
                        default: "Could not launch a new instance."
                    )
    ...
    /// Settings > Validate shortcuts.xml...: pick a shortcuts.xml file and
    /// report structural and key-mapping issues found by the core validator.
    @objc func validateShortcutsXML(_ sender: Any?) {
        let panel = NSOpenPanel()
        panel.allowedContentTypes = [.xml]
        panel.title = Localization.string(.appValidateShortcutsXML, default: "Validate shortcuts.xml...")
        panel.canChooseFiles = true
        panel.canChooseDirectories = false
        panel.allowsMultipleSelection = false
        guard panel.runModal() == .OK, let url = panel.url else { return }

        let report: ShortcutsXMLCodec.ValidationReport
        do {
            let data = try Data(contentsOf: url)
            report = ShortcutsXMLCodec.validate(data)
        } catch {
            let alert = NSAlert()
            alert.alertStyle = .warning
        ...
        if report.isValid {
            ...
            alert.informativeText = String(
                format: Localization.string(
                    .shortcutsXMLValidationPassed,
                    default: "Validation passed: %d shortcuts, no issues found."
                ),
            ...
            let header = String(
                format: Localization.string(
                    .shortcutsXMLValidationFailed,
                    default: "Validation found %d issue(s) in %d shortcuts:"
                ),
                report.issues.count,
                report.shortcutCount
            )
            let detail = report.issues.prefix(20).map(\.message).joined(separator: "\n")
            let truncated = report.issues.count > 20 ? "\n…" : ""
            alert.informativeText = header + "\n\n" + detail + truncated
        }
        alert.runModal()
    }

    /// Replaces the found-results list and brings the results panel forward
    /// (used by the Find dialog's "Find All ..." buttons).
    func publishFoundResults(_ matches: [FindInFilesMatch], purgeFirst: Bool = true) {
        findInFilesResultsStore.setResults(matches, purgeFirst: purgeFirst)
        showFoundResultsPanel()
    }

    /// Locates every match of `query` across all open documents.
    func findAllInOpenDocuments(query: String, options: TextSearch.Options) -> [FindInFilesMatch] {
        var results: [FindInFilesMatch] = []
        for controller in orderedWindows() {
            let path = controller.sessionFileURL?.path ?? controller.windowListSortName
            results.append(contentsOf: DocumentMatchLocator.matches(
                query: query,
                in: controller.editorSurface.text,
                filePath: path,
                options: options
            ))
        }
    ...
    // MARK: - Update channel (GitHub Releases)

    private var updateRepositorySlug: String? {
        if let override = UserDefaults.standard.string(
            forKey: UpdateChecker.repositoryOverrideDefaultsKey
        ), !override.trimmingCharacters(in: .whitespaces).isEmpty {
            return override
        }
        if let fromInfo = Bundle.main.object(
            forInfoDictionaryKey: UpdateChecker.defaultRepositorySlugInfoKey
    ...
    @objc func checkForUpdates(_ sender: Any?) {
        guard let slug = updateRepositorySlug,
              let feedURL = UpdateChecker.releasesAPIURL(repositorySlug: slug)
        else {
            // No release channel configured yet: explain and offer the
            // downloads page, matching the documented interim behavior.
            let alert = NSAlert()
            alert.messageText = Localization.string(
        ...
        if let proxyDictionary = UpdaterProxyStore().load().connectionProxyDictionary {
    ...
    private func presentUpdateCheckResult(
        ...
        if let error {
            alert.alertStyle = .warning
            alert.messageText = Localization.string(
                .helpUpdateCheckFailedTitle, default: "Update check failed")
            alert.informativeText = error.localizedDescription
            alert.runModal()
            return
        }
        if let http = response as? HTTPURLResponse, http.statusCode == 404 {
            alert.messageText = Localization.string(
                .helpUpdateNoReleasesTitle, default: "No releases published")
            alert.informativeText = Localization.string(
                .helpUpdateNoReleasesBody,
                default: "The configured repository has no published releases yet.")
            alert.runModal()
            return
        }
        guard let data, let http = response as? HTTPURLResponse, http.statusCode == 200 else {
            ...
            alert.messageText = Localization.string(
                .helpUpdateCheckFailedTitle, default: "Update check failed")
            alert.informativeText = Localization.string(
                .helpUpdateUnexpectedResponse, default: "The update feed returned an unexpected response.")
            alert.runModal()
            return
        }
        do {
            let releases = try UpdateChecker.decodeReleases(from: data)
            switch try UpdateChecker.evaluate(currentVersion: currentVersion, releases: releases) {
            case .upToDate:
                alert.messageText = Localization.string(
                    .helpUpdateUpToDateTitle, default: "You're up to date")
                alert.informativeText = String(
                    format: Localization.string(
                        .helpUpdateUpToDateBody, default: "Version %@ is the latest published release."),
                    currentVersion
                )
                alert.runModal()
            ...
            case .updateAvailable(let release):
                alert.messageText = Localization.string(
                    .helpUpdateAvailableTitle, default: "Update available")
                alert.informativeText = String(
                    format: Localization.string(
                        .helpUpdateAvailableBody,
                        default: "Version %@ is available (you have %@)."),
                    release.tagName,
                    currentVersion
                )
                alert.addButton(withTitle: Localization.string(
                    .helpUpdateDownload, default: "Download"))
                alert.addButton(withTitle: Localization.string(.alertCancel, default: "Cancel"))
                if alert.runModal() == .alertFirstButtonReturn,
                   let target = release.dmgAssetURL ?? release.htmlURL {
    ...
    @objc func setUpdaterProxy(_ sender: Any?) {
        let store = UpdaterProxyStore()
        let current = store.load()

        let alert = NSAlert()
        alert.messageText = Localization.string(
            .helpSetUpdaterProxyTitle, default: "Set Updater Proxy")
        alert.informativeText = Localization.string(
            .helpSetUpdaterProxyBody,
            default: "The proxy is used only for update checks.")

        let container = NSStackView(frame: NSRect(x: 0, y: 0, width: 280, height: 80))
        container.orientation = .vertical
        container.alignment = .leading
        container.spacing = 8

        let enabledCheckbox = NSButton(
            checkboxWithTitle: Localization.string(
                .helpSetUpdaterProxyEnable, default: "Use a proxy for update checks"),
            target: nil,
            action: nil
        )
        enabledCheckbox.state = current.isEnabled ? .on : .off

        let hostField = NSTextField(string: current.host)
        hostField.placeholderString = Localization.string(
            .helpSetUpdaterProxyHostPlaceholder, default: "Proxy host (e.g. 127.0.0.1)")
        hostField.frame = NSRect(x: 0, y: 0, width: 280, height: 24)

        let portField = NSTextField(string: String(current.port))
        portField.placeholderString = Localization.string(
            .helpSetUpdaterProxyPortPlaceholder, default: "Port")
        portField.frame = NSRect(x: 0, y: 0, width: 280, height: 24)

        container.addArrangedSubview(enabledCheckbox)
        container.addArrangedSubview(hostField)
        container.addArrangedSubview(portField)
        hostField.widthAnchor.constraint(equalToConstant: 280).isActive = true
        portField.widthAnchor.constraint(equalToConstant: 280).isActive = true
        ...
        let settings = UpdaterProxySettings(
            isEnabled: enabledCheckbox.state == .on,
            host: hostField.stringValue.trimmingCharacters(in: .whitespaces),
            port: Int(portField.stringValue.trimmingCharacters(in: .whitespaces)) ?? current.port
        )
        store.save(settings)
    }
    ...
    private func show(_ controller: EditorWindowController, persistSession: Bool) {
        ...
        controller.onDirtyStateChange = { [weak self] in
            // Refresh saved/unsaved tab icons as soon as the flag flips;
            // keep the currently active tab selection unchanged.
            self?.rebuildTabState()
        }
        ...
        controller.onTabPeek = { [weak self] identity, screenRect in
            guard let self else { return }
            guard let identity, let target = self.windows.first(where: { $0.tabIdentity == identity }),
                  target !== self.activeEditorController()
            else {
                self.documentPeeker.hide()
                return
            }
            self.documentPeeker.show(
                title: target.windowListSortName,
                previewText: DocumentPeekerController.previewExcerpt(from: target.editorSurface.text),
                near: screenRect
            )
        }
    ...
    /// View > Move to Other View: parks the active document into the
    /// neighboring tab's window as a second pane (upstream dual-view
    /// semantics mapped onto the shared-tab-strip window model). Invoking
    /// it again from the host window moves the document back out.
    @objc func moveToOtherView(_ sender: Any?) {
        guard let current = activeEditorController() else {
            NSSound.beep()
            return
        }
        // Host window: collapse the pane and reopen the parked document.
        if current.hostedController != nil {
            let released = current.releaseHostedDocument(reactivateReleased: true)
            rebuildTabState()
            if let released { activate(released) }
            return
        }
        let ordered = orderedWindows()
        guard ordered.count >= 2,
              let currentIndex = ordered.firstIndex(where: { $0 === current })
        else {
            NSSound.beep()
            return
        }
        // Prefer the previous tab as host, then the next, skipping windows
        // already involved in a clone or hosted pane.
        let eligible: (EditorWindowController) -> Bool = {
            $0 !== current && $0.hostingController == nil && !$0.secondPaneExists
        }
        let before = ordered[..<currentIndex].reversed().first(where: eligible)
        let after = ordered[(currentIndex + 1)...].first(where: eligible)
        guard let host = before ?? after else {
            NSSound.beep()
            return
        }
        guard host.hostDocumentInOtherView(from: current) else {
            NSSound.beep()
            return
        }
        activate(host)
        host.focusHostedPane()
        rebuildTabState()
    }

    private func activate(_ controller: EditorWindowController) {
        // A parked document lives in its host window's second pane.
        if let host = controller.hostingController {
            activate(host)
            host.focusHostedPane()
            return
        }
    ...
    private func openFolderRecursively(_ folderURL: URL) {
        ...
        if fileURLs.count > 200 {
            let alert = NSAlert()
            alert.messageText = String(
                format: Localization.string(.fileOpenFolderManyTitle, default: "Open %d files?"),
                fileURLs.count
            )
            alert.informativeText = String(
                format: Localization.string(
                    .fileOpenFolderManyMessage,
                    default: "The folder '%@' contains %d files. Opening all of them may slow down the application."
                ),
