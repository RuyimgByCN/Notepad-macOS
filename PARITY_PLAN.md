...
## 对齐上游 v8.9.6.4：ANSI Escape Sequence 词法分析（2026-06-13）✅ 已完成

上游 v8.9.6.4 唯一的功能性提交是 "Add full ANSI EscapeSequence lexing"
（`IDM_LANG_ESCSEQ`，语言 `escseq`，扩展名 `.ans`）。本项目此前已具备全部
前置依赖（`LexEscSeq.cxx` 随 liblexilla 编译、`langs.model.xml`/
`stylers.model.xml` 含 escseq 条目与 16 项 ANSI COLOR 样式、
LanguageCatalog 显示名、`.ans` 解码偏好），唯独 `ScintillaLexilla.lexerNames`
缺 `"escseq"` 映射，导致回退到原生轻量高亮、LexEscSeq 词法器闲置。补一行
映射后端到端打通（样式走 `EditorSurface.applyStyles` 通用路径，按名字从
`StyleCatalog` 取样式经 SCI 应用）。新增 upstreamMappings 测试断言；
swift test 490 项全过。
